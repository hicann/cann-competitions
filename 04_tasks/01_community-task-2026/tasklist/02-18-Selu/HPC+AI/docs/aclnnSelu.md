...
- 计算公式：
  ...
  \begin{cases}
  input_i, & input_i > 0 \\
  \alpha \cdot (\exp(input_i) - 1), & input_i \leq 0
  \end{cases}
  $$
...
- 算子分为[两段式接口](../../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnSeluGetWorkspaceSize”接口获取入参并根据流程计算所需workspace大小，再调用“aclnnSelu”接口执行计算。
